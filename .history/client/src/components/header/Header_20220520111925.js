...
const setAsyncCurrentUserData = async () => {
      try {
        //setUserEmail(getAuth().currentUser.email);
        //setUserEmail(getAuth().currentUser.email);
        setCurrentUser({value: currentUser.value, isFetching: true})
        //const userEmailValue = getAuth().currentUser.email;
        console.log("ce e aici" + userEmail.value);
        const userValue = await axios.get('http://127.0.0.1:5000/api/users', { params: { email: userEmail.value } })
        console.log("user email value " + userValue);
        if(!userValue) {
